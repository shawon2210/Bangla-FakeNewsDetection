"""
Experiment 3 (Enhanced): EfficientNet-B3 Image-Only Ablation v3.
Uses enhanced image encoder with strong augmentation, 30 epochs.
"""
...
from model_v3_enhanced import ImageOnlyEnhanced
from data_v3_enhanced import ImageOnlyDataset, get_train_transforms, get_eval_transforms
import pandas as pd
...
print("=" * 60)
print("EXPERIMENT 3 (Enhanced): EfficientNet-B3 Image-Only Ablation v3")
print("=" * 60)
...
train_ds = ImageOnlyDataset(train_df, get_train_transforms(), IMG_DIR)
val_ds = ImageOnlyDataset(val_df, get_eval_transforms(), IMG_DIR)
test_ds = ImageOnlyDataset(test_df, get_eval_transforms(), IMG_DIR)
...
# Model
model = ImageOnlyEnhanced(num_classes=2, feature_dim=512).to(DEVICE)
    ...
    for b in tqdm(train_dl, desc=f"  E{ep+1}/{EPOCHS}", leave=False):
        ...
        if USE_AMP:
            with torch.cuda.amp.autocast():
                logits = model(images=b["image"].to(DEVICE))
...
print(f"\n{'='*60}\nEfficientNet-B3 Image-Only Ablation — TEST RESULTS")
...
res = {
    "experiment": "EfficientNet-B3 Image-Only Ablation v3 (Ours)",
...
with open(os.path.join(SAVE_DIR, "results_exp3_enhanced.json"), "w") as f:
    json.dump(res, f, indent=2)
print("Saved results_exp3_enhanced.json")
